...
1. **认证授权中心** (auth-server)
2. **鉴权网关** (auth-gateway)  
3. **系统服务** (luc-system)
4. **开发脚手架** (luc-framework)
...
- `/user/detail` - 获取用户详情
- `/user/info` - 获取用户信息
- `/institution/info` - 获取机构信息

**配置信息:**
- 端口: 19003
- 数据库: institution_center
...
### 认证流程
1. 用户通过 auth-gateway 统一访问系统
2. 请求头携带 `x-access-token` 标识已登录用户
3. auth-gateway 和 auth-server 通过 `x-access-token` 从 Redis 获取安全上下文和 JWT Token
4. auth-server 支持 OAuth2.0 认证流程，扩展多种登录方式
...
### 数据库配置
- **auth-server**: authorization_server 数据库
- **auth-gateway**: institution_center 数据库  
- **luc-system**: institution_center 数据库
