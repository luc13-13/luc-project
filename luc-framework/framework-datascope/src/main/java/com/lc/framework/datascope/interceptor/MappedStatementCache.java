...
public class MappedStatementCache {
     ...
     */
    private static final Map<Class<? extends IDataScopeSqlHandler>, Set<String>> NO_REWRITE_MAP_STATEMENTS = new ConcurrentHashMap<>();
    ...
    public static void addIgnoredMapStatement(MappedStatement ms, List<IDataScopeSqlHandler> handlers) {
        if (DataScopeContextHolder.isRewrite()) {
            return;
        }
        for (IDataScopeSqlHandler handler : handlers) {
    ...
    public static boolean willIgnored(MappedStatement ms, List<IDataScopeSqlHandler> handlers) {
