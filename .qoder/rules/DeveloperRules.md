...
(2) 请求头中携带luc_auth_token视为已登陆用户，authorization-gateway和authorization-server通过luc_auth_token作为key，从redis中获取security context; 请求头中的x-access-token携带了用户信息，由jwt解码
(3) authorization-server支持OAuth2.0的认证流程，同时拓展短信登陆、gitee登陆、微信登陆
